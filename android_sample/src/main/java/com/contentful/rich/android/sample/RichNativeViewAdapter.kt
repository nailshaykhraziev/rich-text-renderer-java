...
class RichNativeViewAdapter(
    pageIndex: Int,
    private val androidContext: Context
) : RecyclerView.Adapter<PageFragment.Holder>() {
    ...
    private val context = AndroidContext(
        androidContext, AndroidConfig(
            textColor = android.R.color.holo_red_dark,
            linkColor = ContextCompat.getColor(androidContext, android.R.color.holo_green_light),
            marginTop = 8
        )
    )
    ...
    override fun onBindViewHolder(holder: PageFragment.Holder, position: Int) {
        ...
        val view =
            processor.process(context, item) ?: TextView(this.androidContext).apply { setText(R.string.error_no_view) }
